...
function wireRoutePage() {
  ...
  document.getElementById("route-page-ov-search-btn").addEventListener("click", async () => {
    ...
    try {
      await runOvSearch(statusEl, listEl);
    } catch (e) {
      console.error("OV-zoekopdracht mislukt", e);
      statusEl.classList.remove("hidden");
      statusEl.textContent = "Er ging iets mis: " + (e.message || "onbekende fout");
    }
  });

  async function runOvSearch(statusEl, listEl) {
    ...
    async function resolveFromLocation() {
      if (fromText) {
        statusEl.textContent = "Vertrekadres opzoeken...";
        return await geocodeAddress(fromText);
      }
      statusEl.textContent = "Je huidige locatie wordt opgevraagd...";
    ...
    const fromLocation = await resolveFromLocation();
    if (!fromLocation) {
      statusEl.textContent = fromText
        ? "Kon dit vertrekadres niet vinden."
        : "Kon je locatie niet ophalen — vul eventueel een vertrekadres in.";
      return;
    }

    let toLocation = null;
    if (toText) {
      statusEl.textContent = "Bestemming opzoeken...";
      toLocation = await geocodeAddress(toText);
    }

    statusEl.textContent = "Haltes zoeken...";
    await ensureOvStopsLoaded();
    if (!ovStopsCache || ovStopsCache.length === 0) {
      statusEl.textContent = "Kon de haltelijst niet ophalen — probeer het later opnieuw.";
      return;
    }

    function nearestStops(location, count) {
      return ovStopsCache
        .map(s => ({ ...s, distance: haversineDistanceMeters(location.lat, location.lon, s.lat, s.lon) }))
        .sort((a, b) => a.distance - b.distance)
        .slice(0, count);
    }

    function stopsWithinRadius(location, radiusMeters, maxCount) {
      return ovStopsCache
        .map(s => ({ ...s, distance: haversineDistanceMeters(location.lat, location.lon, s.lat, s.lon) }))
    ...
    const fromStops = stopsWithinRadius(fromLocation, 5000, 20);
    statusEl.textContent = "Vertrektijden ophalen...";
    const perStop = await Promise.all(fromStops.map(async (stop) => ({ stop, passes: await fetchOvDepartures(stop.code) })));
    ...
    perStop.forEach(({ stop, passes }) => {
      passes.forEach(p => {
        ...
        li.innerHTML = `
          ...
          <div class="restaurant-cuisine">Vanaf ${escapeHtml(stop.name)} (${Math.round(stop.distance)}m)</div>
        `;
    ...
    if (toLocation) {
      const toStops = nearestStops(toLocation, 2);
      const toHeader = document.createElement("li");
      toHeader.className = "restaurant-row";
      toHeader.innerHTML = `<div class="restaurant-cuisine" style="font-weight:600;">Dichtstbijzijnde halte(s) bij bestemming:</div>`;
      listEl.appendChild(toHeader);
      toStops.forEach(stop => {
        any = true;
        const li = document.createElement("li");
        li.className = "restaurant-row";
        li.innerHTML = `<div class="restaurant-cuisine">${escapeHtml(stop.name)} (${Math.round(stop.distance)}m van bestemming)</div>`;
        listEl.appendChild(li);
      });
    }

    if (!any) { statusEl.classList.remove("hidden"); statusEl.textContent = "Geen haltes gevonden binnen 5 km van dit adres."; }
  }
