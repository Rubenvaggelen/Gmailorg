...
function boot() {
  ...
  // Ondersteunt een directe link naar het Route-tabblad (bv. vanuit "The
  // One"-app se eigen "Route"-tegel): #route in de URL opent dit tabblad meteen.
  // #route-standalone doet hetzelfde, maar verbergt ook de hele Mail/Kalender-
  // navigatie eromheen, zodat Route als volledig losse pagina voelt.
  if (window.location.hash === "#route" || window.location.hash === "#route-standalone") {
    const routeTab = document.querySelector('.tab[data-view="route"]');
    if (routeTab) routeTab.click();
  }
  if (window.location.hash === "#route-standalone") {
  ...
  // Meteen bij opstarten ophalen, in plaats van te wachten tot een token
  // ververst, de ververs-timer afloopt, of er handmatig op vernieuwen wordt
  // gedrukt — zo staan alle accounts direct zichtbaar bij het openen.
  refreshInbox();
  refreshCalendar();
...
function decodeBase64Url(data) {
  ...
  try {
    // TextDecoder decodeert UTF-8 (incl. emoji en andere meerbytetekens)
    // correct — de oudere escape()/unescape()-truc laat zulke tekens als
    // kapotte "�"-blokjes zien.
...
function stripHtml(html) {
  ...
  d.innerHTML = html;
  // <style>- en <script>-inhoud zit qua DOM gewoon als tekst in het element,
...
/** Opent Waze met de bestemming al ingevuld, voor turn-by-turn navigatie. */
function openWazeNavigation() {
  if (!navDestination) return;
  const url = `https://waze.com/ul?ll=${navDestination.lat},${navDestination.lon}&navigate=yes`;
  window.open(url, "_blank");
}

/** Opent Google Maps met de bestemming al ingevuld, voor turn-by-turn navigatie. */
function openGoogleMapsNavigation() {
  if (!navDestination) return;
  const url = `https://www.google.com/maps/dir/?api=1&destination=${navDestination.lat},${navDestination.lon}&travelmode=driving`;
  window.open(url, "_blank");
}
...
function wireRouteMapModal() {
  ...
  document.getElementById("route-nav-start-btn").addEventListener("click", openWazeNavigation);
  document.getElementById("route-nav-gmaps-btn").addEventListener("click", openGoogleMapsNavigation);
}
...
function wireRoutePage() {
  ...
  document.getElementById("route-page-ov-search-btn").addEventListener("click", async () => {
    const statusEl = document.getElementById("route-page-ov-status");
    const fromText = document.getElementById("route-page-ov-from").value.trim();
    const toText = document.getElementById("route-page-ov-to").value.trim();

    if (!toText) {
      statusEl.classList.remove("hidden");
      statusEl.textContent = "Vul een bestemming in bij 'Naar'.";
      return;
    }
    ...
    if (fromText) params.set("origin", fromText);
...
/* ---------------- Goedkoopste tanken (los blokje onder Route) ----------------
   Databron: de publieke (niet-officiële, reverse-engineered) REST API die de
   ANWB-app zelf gebruikt. Geen eigen account nodig, maar ook niet officieel
   gedocumenteerd/ondersteund — de sleutel wordt door meerdere open-source
   projecten gedeeld (o.a. github.com/bartmachielsen/ANWB-Fuel-Prices) en kan
   op enig moment door ANWB ingetrokken of gewijzigd worden. */

const FUEL_API_BASE = "https://api.anwb.nl/v1";
const FUEL_API_KEY = "GNYXLHxCa14zfA1nVJkVV8ldQZHbPTcu";
const FUEL_SEARCH_RADIUS_KM = 10;
const FUEL_MAX_RESULTS = 5;

let fuelTypesCache = null;
let selectedFuelTypeId = null;
let fuelStationsRaw = [];
let fuelOrigin = null;

function fuelBoundingBox(lat, lng, radiusKm) {
  const latDelta = radiusKm / 111;
  const lngDelta = radiusKm / (111 * Math.cos(lat * Math.PI / 180));
  return { sw: [lat - latDelta, lng - lngDelta], ne: [lat + latDelta, lng + lngDelta] };
}
...
function extractFuelPrice(station) {
  ...
  for (const p of candidates) {
    ...
    if (typeId === selectedFuelTypeId) return p.price ?? p.value ?? p.amount ?? null;
  }
...
function renderFuelTypeRow() {
  const row = document.getElementById("route-fuel-type-row");
  row.innerHTML = "";
  (fuelTypesCache || []).forEach(t => {
    const chip = document.createElement("button");
    chip.className = "chip" + (t.id === selectedFuelTypeId ? " active" : "");
    chip.textContent = t.name || t.description || ("Type " + t.id);
    chip.addEventListener("click", () => {
...
async function searchFuelStations() {
  const statusEl = document.getElementById("route-fuel-status");
  const listEl = document.getElementById("route-fuel-results");
  statusEl.classList.remove("hidden");
  statusEl.textContent = "Locatie ophalen...";
  listEl.innerHTML = "";

  try {
    fuelOrigin = await resolveOrigin();

    if (!fuelTypesCache) {
      const data = await fuelApiGet("/fuel/types");
      fuelTypesCache = data.items || data || [];
      const e95 = fuelTypesCache.find(t => (t.name || t.description || "").toLowerCase().includes("95"));
      selectedFuelTypeId = e95 ? e95.id : (fuelTypesCache[0] && fuelTypesCache[0].id);
      renderFuelTypeRow();
    }

    statusEl.textContent = "Prijzen ophalen...";
    const { sw, ne } = fuelBoundingBox(fuelOrigin.latitude, fuelOrigin.longitude, FUEL_SEARCH_RADIUS_KM);
    const data = await fuelApiGet(`/fuel/stations?sw=${sw[0]},${sw[1]}&ne=${ne[0]},${ne[1]}`);
    fuelStationsRaw = (data.items || data || []).map(s => {
      const coords = getFuelStationCoords(s);
      return {
        raw: s,
        ...
        distanceKm: coords
          ? haversineDistanceMeters(fuelOrigin.latitude, fuelOrigin.longitude, coords[0], coords[1]) / 1000
          : null
      };
    });
...
function renderFuelResults() {
  ...
  const withPrice = fuelStationsRaw
    .map(s => ({ ...s, price: extractFuelPrice(s.raw) }))
    .filter(s => s.price != null)
    .sort((a, b) => a.price - b.price)
    .slice(0, FUEL_MAX_RESULTS);

  if (withPrice.length === 0) {
    statusEl.classList.remove("hidden");
    statusEl.textContent = "Geen prijzen gevonden binnen " + FUEL_SEARCH_RADIUS_KM + " km.";
    return;
  }
  statusEl.classList.add("hidden");
  ...
  withPrice.forEach(s => {
    ...
    li.innerHTML = `
      <div class="restaurant-top">
        <span class="restaurant-name">${escapeHtml(s.name)}</span>
        <span class="restaurant-distance">€${s.price.toFixed(3)}</span>
      </div>
      <div class="restaurant-cuisine">${escapeHtml(s.brand)}</div>
      <div class="restaurant-hours">${s.distanceKm != null ? s.distanceKm.toFixed(1) + " km" : ""}</div>
    `;
    if (s.lat != null && s.lon != null) {
      li.addEventListener("click", () => openFuelStationRoute(s));
    }
    listEl.appendChild(li);
  });
}

/** Berekent de autoroute naar het gekozen tankstation en toont 'm in het
    bestaande route-map-modal, met dezelfde Waze/Google Maps-knoppen als
    de Auto-route hierboven. */
async function openFuelStationRoute(station) {
  const statusEl = document.getElementById("route-fuel-status");
  statusEl.classList.remove("hidden");
  statusEl.textContent = "Route berekenen...";
  try {
    const routeR = await fetch(
      `https://router.project-osrm.org/route/v1/driving/${fuelOrigin.longitude},${fuelOrigin.latitude};${station.lon},${station.lat}?overview=full&geometries=geojson&steps=true`
    );
    const routeData = await routeR.json();
    const route = routeData.routes?.[0];
    if (!route) { statusEl.textContent = "Kon geen route berekenen."; return; }
    statusEl.classList.add("hidden");
    showRouteOnMap(fuelOrigin, { lat: station.lat, lon: station.lon }, route.geometry, station.name, route.legs?.[0]?.steps || []);
  } catch (e) {
    console.error("Route naar tankstation mislukt", e);
    statusEl.classList.remove("hidden");
    statusEl.textContent = "Kon geen route berekenen.";
  }
}
...
const SETTINGS_PIN = "290114";
let settingsUnlocked = false;

function wireSettingsLock() {
  const tryUnlock = () => {
    ...
    if (input.value === SETTINGS_PIN) {
      ...
      document.getElementById("settings-lock").classList.add("hidden");
  ...
  document.getElementById("settings-unlock-btn").addEventListener("click", tryUnlock);
  ...
  document.querySelectorAll(".tab").forEach(tab => {
    tab.addEventListener("click", () => {
      ...
      if (view === "settings" && !settingsUnlocked) {
        document.getElementById("settings-lock").classList.remove("hidden");
